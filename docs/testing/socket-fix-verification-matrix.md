...
plane over the same image, container, pod, and runtime operations.

## Preconditions

- No `DOCKER_HOST` override unless explicitly testing compatibility mode.
...
| Rust workspace | `cargo test --workspace -- --test-threads=1` | Core, CLI, runtime helper, and GUI backend tests pass. |
| Frontend build | `pnpm run build` from `crates/cratebay-gui` | Production frontend bundle builds. |
| Frontend unit tests | `pnpm run test:unit` from `crates/cratebay-gui` | App, stores, containers, pods, settings, and runtime health tests pass. |
| Desktop E2E | `pnpm exec playwright test` from `crates/cratebay-gui` | Images, containers, pods, settings, and navigation flows pass. |
...
- Docker-compatible endpoint readiness from the built-in runtime
...
- cleanup of containers, pods, volumes, images, runtime data, and test sockets
...
default product path. Test them only by setting `DOCKER_HOST` or passing
`--docker-host`.
