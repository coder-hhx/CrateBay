//! Integration tests for cargobay-core.
//!
...
fn core_config_and_data_dirs_are_consistent() {
    let _lock = env_lock();
    let tmp = TempDir::new("cargobay-int-test");
    let _g1 = EnvGuard::set(
        "CARGOBAY_CONFIG_DIR",
        tmp.path.join("config").to_str().unwrap(),
    );
    let _g2 = EnvGuard::set("CARGOBAY_DATA_DIR", tmp.path.join("data").to_str().unwrap());
    let _g3 = EnvGuard::set("CARGOBAY_LOG_DIR", tmp.path.join("logs").to_str().unwrap());

    let config = cargobay_core::config_dir();
    let data = cargobay_core::data_dir();
    let log = cargobay_core::log_dir();
...
fn core_platform_info_is_not_empty() {
    let info = cargobay_core::platform_info();
    assert!(
        !info.is_empty(),
...
fn stub_hypervisor_create_and_list() {
    use cargobay_core::hypervisor::{Hypervisor, VmConfig, VmState};
    use cargobay_core::vm::StubHypervisor;

    let _lock = env_lock();
    let tmp = TempDir::new("cargobay-stub-hv");
    let _g1 = EnvGuard::set("CARGOBAY_CONFIG_DIR", tmp.path.to_str().unwrap());
    let _g2 = EnvGuard::set("CARGOBAY_DATA_DIR", tmp.path.to_str().unwrap());
...
fn stub_hypervisor_lifecycle() {
    ...
    let _lock = env_lock();
    let tmp = TempDir::new("cargobay-lifecycle");
    let _g1 = EnvGuard::set("CARGOBAY_CONFIG_DIR", tmp.path.to_str().unwrap());
    let _g2 = EnvGuard::set("CARGOBAY_DATA_DIR", tmp.path.to_str().unwrap());
...
fn stub_hypervisor_duplicate_name_fails() {
    use cargobay_core::hypervisor::{Hypervisor, VmConfig};
    use cargobay_core::vm::StubHypervisor;

    let _lock = env_lock();
    let tmp = TempDir::new("cargobay-dup-name");
    let _g1 = EnvGuard::set("CARGOBAY_CONFIG_DIR", tmp.path.to_str().unwrap());
    let _g2 = EnvGuard::set("CARGOBAY_DATA_DIR", tmp.path.to_str().unwrap());
...
fn stub_hypervisor_virtiofs_mount_lifecycle() {
    use cargobay_core::hypervisor::{Hypervisor, SharedDirectory, VmConfig};
    use cargobay_core::vm::StubHypervisor;

    let _lock = env_lock();
    let tmp = TempDir::new("cargobay-virtiofs");
    let _g1 = EnvGuard::set("CARGOBAY_CONFIG_DIR", tmp.path.to_str().unwrap());
    let _g2 = EnvGuard::set("CARGOBAY_DATA_DIR", tmp.path.to_str().unwrap());
...
fn stub_hypervisor_port_forward_lifecycle() {
    use cargobay_core::hypervisor::{Hypervisor, PortForward, VmConfig};
    use cargobay_core::vm::StubHypervisor;

    let _lock = env_lock();
    let tmp = TempDir::new("cargobay-portfwd");
    let _g1 = EnvGuard::set("CARGOBAY_CONFIG_DIR", tmp.path.to_str().unwrap());
    let _g2 = EnvGuard::set("CARGOBAY_DATA_DIR", tmp.path.to_str().unwrap());
...
fn image_catalog_and_find_work_together() {
    let catalog = cargobay_core::images::builtin_catalog();
    for entry in &catalog {
        let found = cargobay_core::images::find_image(&entry.id);
        assert!(
            found.is_some(),
...
fn image_paths_are_under_data_dir() {
    let _lock = env_lock();
    let tmp = TempDir::new("cargobay-img-paths");
    let _g = EnvGuard::set("CARGOBAY_DATA_DIR", tmp.path.to_str().unwrap());

    let paths = cargobay_core::images::image_paths("alpine-3.19");
    let data_dir = cargobay_core::data_dir();
...
fn k3s_kubeconfig_path_under_data_dir() {
    let _lock = env_lock();
    let tmp = TempDir::new("cargobay-k3s-int");
    let _g1 = EnvGuard::set("CARGOBAY_DATA_DIR", tmp.path.to_str().unwrap());
    let _g2 = EnvGuard::set("CARGOBAY_CONFIG_DIR", tmp.path.to_str().unwrap());

    let kc = cargobay_core::k3s::K3sManager::kubeconfig_path();
    let data = cargobay_core::data_dir();
    assert!(
        kc.starts_with(&data),
...
fn vm_console_log_path_is_under_data_dir() {
    let _lock = env_lock();
    let tmp = TempDir::new("cargobay-console");
    let _g = EnvGuard::set("CARGOBAY_DATA_DIR", tmp.path.to_str().unwrap());

    let path = cargobay_core::vm_console_log_path("test-vm-42");
    let data = cargobay_core::data_dir();
    assert!(
...
fn stub_hypervisor_persists_vms_across_instances() {
    ...
    let _lock = env_lock();
    let tmp = TempDir::new("cargobay-persist");
    let _g1 = EnvGuard::set("CARGOBAY_CONFIG_DIR", tmp.path.to_str().unwrap());
    let _g2 = EnvGuard::set("CARGOBAY_DATA_DIR", tmp.path.to_str().unwrap());
