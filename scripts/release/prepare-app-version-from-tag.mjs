...
function usage() {
  return [
    ...
    "",
    "Options:",
    "  --github-env <path>      Append CRATEBAY_* variables for later workflow steps.",
...
function updaterConfig(releaseTag) {
  const repository = (
  ...
  const publicKey = (
    process.env.CRATEBAY_UPDATER_PUBLIC_KEY ||
  ...
  return {
    plugins: {
      updater: {
        pubkey: publicKey,
        endpoints: [
          `https://github.com/${repository}/releases/download/${encodeURIComponent(releaseTag)}/latest.json`,
        ],
      },
    },
  };
...
function writeTauriConfig(path, appVersion, releaseTag) {
  mkdirSync(dirname(path), { recursive: true });
  const overlay = {
    ...tauriVersionConfig(appVersion),
    ...updaterConfig(releaseTag),
  };
...
try {
  ...
  const metadata = parseReleaseVersion(
    options.releaseTag || process.env.CRATEBAY_RELEASE_TAG || process.env.RELEASE_TAG,
  );

  if (options.tauriConfigPath) {
    writeTauriConfig(options.tauriConfigPath, metadata.appVersion, metadata.releaseTag);
  }

  if (options.githubEnvPath) {
    const envLines = [
      `CRATEBAY_RELEASE_TAG=${metadata.releaseTag}`,
      `CRATEBAY_APP_VERSION=${metadata.appVersion}`,
      `CRATEBAY_IS_PRERELEASE=${metadata.isPrerelease}`,
    ];
    if (options.tauriConfigPath) {
      envLines.push(`CRATEBAY_TAURI_VERSION_CONFIG=${options.tauriConfigPath}`);
    }
  ...
  if (options.json) {
    ...
    const configSuffix = options.tauriConfigPath
      ...
      : "";
    console.log(
      `Prepared CrateBay ${metadata.releaseTag} (app version ${metadata.appVersion}, prerelease ${metadata.isPrerelease}).${configSuffix}`,
    );
  }
} catch (error) {
